...
def _windowed_subdivs(padded_img, window_size, subdivisions, nb_classes, pred_func,ratio):
    """
    ...
    """
    WINDOW_SPLINE_2D = _window_2D(window_size=int(window_size/ratio), power=2)#change here
    ...
    print('input shape',subdivs.shape)
    subdivs = subdivs.reshape(a * b, c, d, e)
    gc.collect()

    subdivs = pred_func(subdivs)
    print('out shape',subdivs.shape)
    c, d = subdivs.shape[1:3] #new shape this is wrong
    gc.collect()
...
def _recreate_from_subdivs(subdivs, window_size, subdivisions, padded_out_shape, ratio):
    """
    Merge tiled overlapping patches smoothly.
    """
    step = int(window_size/subdivisions) #32/2=16
    padded_out_shape[0]=math.floor(padded_out_shape[0]/ratio)
    padded_out_shape[1]=math.floor(padded_out_shape[1]/ratio)
...
def predict_img_with_smooth_windowing(input_img, window_size, subdivisions, nb_classes, pred_func, ratio):
    """
    ...
    for pad in tqdm(pads):
        # For every rotation:
        sd = _windowed_subdivs(pad, window_size, subdivisions, nb_classes, pred_func, ratio)
        print('padded_out_shape',list(pad.shape[:-1])+[nb_classes])
        one_padded_result = _recreate_from_subdivs(
            sd, int(window_size/ratio), subdivisions,
    ...
    prd = _unpad_img(padded_results, int(window_size/ratio), subdivisions)
